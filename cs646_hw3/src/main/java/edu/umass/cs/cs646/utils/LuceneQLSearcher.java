...
public class LuceneQLSearcher extends AbstractQLSearcher {
    ...
    public static void main(String[] args) {
        try {
            ...
            for (double lamda=0.1;lamda<=0.1;lamda+=0.1) {
                ...
                for (String qid : queries.keySet()) {
                    ...
                    Map<String, Double> resultRM1 = LuceneQLSearcher.estimateQueryModelRM1(searcher.index, searcher.getDocLengthReader(field_search), field_search, terms, mu, muFb, k, n);
                    Map<String, Double> resultMLE = new HashMap<>();
                    for ( String term : terms ) {
                        resultMLE.put( term, resultMLE.getOrDefault( term, 0.0 ) + 1.0 );
                    }

                    Map<String,Double> resultRM3=new HashMap<>();
                    for(String key:resultRM1.keySet())
                    {
                        double rm1=resultRM1.get(key);
                        Double mle=resultMLE.get(key);
                        double rm3;

                        if(mle==null)
                            rm3=(1-lamda)*rm1;
                        else
                            rm3=((1-lamda)*rm1)+(lamda*mle);
                        resultRM3.put(key,rm3);
                    }

                    List<SearchResult> results = searcher.search(field_search, resultRM3, mu, top);
